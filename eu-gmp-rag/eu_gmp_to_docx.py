...
def extract_blocks(pdf_path: Path) -> list[tuple[str, object]]:
        ...
        for pno, page in enumerate(pdf.pages):
            ...
            for ln in lines:
                ...
                items.append((ln["top"], "foot" if foot else ("tabline" if tab else "line"),
                              (clean(ln["text"]).strip(), ln["bottom"], page.height, bold)))
    ...
    for items in pages:
        ...
        for top, typ, p in items:
            ...
            if not text or (PAGE_NO_RE.match(text) and edge) or norm(text) in repeated or NOISE_RE.match(text):
                continue
...
TOC_RE = re.compile(r"^inhaltsverzeichnis$", re.I)


def strip_front_matter(blocks):
...
def topic_of(part) -> str:
    ...
    for t, text in part:
        if t == "h1" and "(Fortsetzung)" not in text:
            h = re.sub(r"\s*\([^)]*\)", "", re.sub(r"^\d{1,2}\.?\s+", "", text))  # "(RBA)" u.ä. weglassen
